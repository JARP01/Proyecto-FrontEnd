...
import TarjetasC from "../Tarjetas/TarjetasC";

const ContenedorTarjetaC = ({ filtro }) => {
  const login = localStorage.getItem("login");
  const [datos, setDatos] = useState([]);
  const ts = 2;
  const key ="320eb76cfe487db609c8392261b2afca";
  const hash = "7c35834cfa5c6dff86b5767c8710eac4";
  const url = `https://gateway.marvel.com/v1/public/events?ts=${ts}&apikey=${key}&hash=${hash}`;
...
export default ContenedorTarjetaC;